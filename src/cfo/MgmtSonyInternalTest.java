...
public class MgmtSonyInternalTest extends BasePage {
...
//	public static Web  = null;		//Web instance created
	...
	{
		extent = new com.relevantcodes.extentreports.ExtentReports("C:\\Users\\Mayuri\\Desktop\\Compliance\\AvacomAll\\Reports\\SonyMGMT.html",true);
		test = extent.startTest("Loging In - MGMT (Internal)- Sony");
	...
	{
		test = extent.startTest(" Internal Compliance Due verification");
		
		
		MgmtSonyMethod.ComplianceDueIn(test);
	...
	@Test(priority = 5)
	void ComplianceDueInFilter() throws InterruptedException, IOException
	{
		test = extent.startTest("To check whether the all the filters are working on the box of Internal compliance due or not");
		
		
		MgmtSonyMethod.ComplianceDueInFilter(test,);
	...
	{
		test = extent.startTest(" Internal Compliance reported verification");
		
		
		MgmtSonyMethod.ComplianceReportedIn(test);
	...
	@Test(priority = 7)
	void CompliancereportedInFilter() throws InterruptedException, IOException
	{
		test = extent.startTest("To check whether the all the filters are working on the box of Internal compliance reported or not");
		
		
		MgmtSonyMethod.CompliancereportedInFilter(test,);
	...
	@Test(priority = 8)
	void ComplianceOverdueIn() throws InterruptedException, IOException
	{
		test = extent.startTest("Internal Overdue Compliance verification");
		
		
		MgmtSonyMethod.ComplianceOverdueIn(test);
	...
	@Test(priority = 9)
	void OverdurComplianceInFilter() throws InterruptedException, IOException
	{
		test = extent.startTest("To check whether the all the filters are working on the box of Internal Overdue compliance or not");
		
		
		MgmtSonyMethod.OverdurComplianceInFilter(test,);
...
int NotApplicable = Integer.parseInt(MgmtSonyMethod.Rejected().getText());	//Reading the High Risk value of Not Completed compliance
	...
	void RiskSummaryCriticalStatutory() throws InterruptedException
	{
		...
		/*Thread.sleep(2000);
		List<WebElement>roc = .findElements(By.xpath("(//*[@class='highcharts-legend-item highcharts-column-series highcharts-color-undefined highcharts-series-0'])"));
	...
	void DepartmentSummaryHumanResourceStatutory() throws InterruptedException
	{
		...
		Thread.sleep(3000);
		WebElement roc = .findElement(By.cssSelector("#highcharts-8 > svg > g.highcharts-legend > g > g > g.highcharts-legend-item.highcharts-bar-series.highcharts-color-undefined.highcharts-series-2 > rect"));
...
By locator = By.xpath("//*[@id='grid']/div[3]/table/tbody/tr/td[6]/a");
	...
	void DailyUpdates() throws InterruptedException, IOException
	{
		...
		Thread.sleep(4000);	
	//	WebWait wait = new WebWait( Duration.ofSeconds(30));
